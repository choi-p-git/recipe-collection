...
def get_item_edit_payload(item_id: int) -> dict | None:
    ...
    return {
        ...
        "volume_unit": row[9],
        "serving_size_quantity": row[10],
        "serving_size_unit": row[11],
        "serving_count": row[12],
        "instruction_steps": decode_instruction_text_to_steps(row[13] or ""),
        "primary_cooking_method_code": row[14],
        "status": row[15],
        "requires_resubmission": bool(row[16]),
        "notes": row[17],
        "concept_classification": row[18],
        "meal_classification": row[19],
        "haccp_process_classification": row[20],
